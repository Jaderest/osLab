...
static task_t *buffers[MAX_CPU_NUM] =
...
Context *kmt_schedule(Event ev, Context *ctx) {
  ...
  if (buffer && buffer != current) {
    atomic_xchg(&buffer->suspend, 0); // 任务不再独占，其他cpu可以运行
  }
  if (buffer->dead == 1 && buffer != current &&
      buffer->cpu_id == cpu_current()) {
    kmt_teardown(buffer);
  }
  ...
  for (i = 0; i < total_task_num * 10; ++i) { // 循环十遍
    ...
    if (tasks[index]->block == 0 && tasks[index]->dead == 0 &&
        tasks[index]->running == 0 &&
        (tasks[index] == current || !atomic_xchg(&tasks[index]->suspend, 1))) {
      // 一个任务可以调度
      // 不阻塞，不死亡，不在运行，不在独占
      break; // 选中
    }
  ...
  atomic_xchg(&current->running, 1); // 当前任务运行
...
void kmt_teardown(task_t *task) {
  PANIC_ON(task->dead != 1, "teardown err");
  _spin_lock(&task_lk);
  task->status = DEAD;
  tasks[task->id] = NULL;
  _spin_unlock(&task_lk);
...
void kmt_sem_wait(sem_t *sem) {
  ...
  if (sem->value < 0) {
    log("if\n");
    // 当前线程不能执行，BLOCKED！
    atomic_xchg(&current->block, 1); // 阻塞
